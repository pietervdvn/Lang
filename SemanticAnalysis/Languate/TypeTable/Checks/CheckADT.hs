...
validateADTSum tlt frees (ADTSum nm _ prods)
		= inside ("In the concstructor "++nm++": ") $ do
			let usedNms = mapMaybe fst prods
			assert (unique usedNms) $
				"A field name should be unique in each constructor. You used "++intercalate ", " (dubbles usedNms) ++" at least twice"
			validateTypes tlt frees $ map snd prods


crossValidate	:: TypeLookupTable -> [Name] -> [ADTSum] -> Check
crossValidate tlt frees sums
		= do	let getProds (ADTSum cons _ prods)	= zip (mapMaybe unpackMaybeTuple prods) $ repeat cons
